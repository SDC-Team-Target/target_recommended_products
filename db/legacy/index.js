...
const addProduct = function (params, cb) {
  ...
  const insertQuery = 'INSERT INTO items (item_id, item_name, category, item_price, img_url) VALUES (?, ?, ?, ?, ?)';
  const {
    item_id, item_name, category, item_price, img_url,
  } = params;
  connection.query(insertQuery, [item_id, item_name, category, item_price, img_url], (err, confirmation) => {
    if (err) {
      cb(err, null);
    } else {
...
const deleteProduct = (id, cb) => {
  ...
  connection.query(deleteQuery, [id], (err, confirmation) => {
    if (err) {
      cb(err, null);
    } else {
      cb(null, confirmation);
...
const updateProduct = (params, cb) => {
  const updateQuery = 'UPDATE items SET item_name = ?, category = ?, item_price = ?, img_url = ? WHERE item_id = ?;';
  const {
    item_name, category, item_price, img_url, item_id,
  } = params;
  // eslint-disable-next-line max-len
  connection.query(updateQuery, [item_name, category, item_price, img_url, item_id], (err, confirmation) => {
