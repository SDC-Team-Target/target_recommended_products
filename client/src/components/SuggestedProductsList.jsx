...
const SuggestedProductsList = ({products}) => {
  ...
  const getRandomProducts = function(array) {
  ...
  return(
    <div>
    <div className="row">
      {products ? shuffledProducts.slice(0, 6).map(product => (<SuggestedProduct product={product} key={product.item_id}/>))
      : <div>item not available</div>}
    </div>
   {(clicked && products) ? (
     <div>
       <div className="row">
       {shuffledProducts.slice(6, 12).map(product => (<SuggestedProduct product={product} key={product.item_id}/>))}
       </div> 
       <button onClick={() => {
